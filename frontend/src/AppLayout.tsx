...
import type { DrawerFilters, FilterState } from './types/filters'
...
function isFilterable(v: string): v is FilterableView {
  return v === 'all' || v === 'recommended' || v === 'favorites' || v === 'applied'
}

function viewFromPath(pathname: string): FilterableView | 'resume' | 'settings' {
...
export default function AppLayout() {
  ...
  // ── Auth modal (kept for any in-app "log in" prompts) ────────────────────────
  ...
  // ── Login prompt (safety net — router already guards, but keeps UI consistent) ─
  ...
  return (
    <div className="flex h-screen bg-[#080b12] text-white antialiased overflow-hidden">
      ...
      {/* In-app auth modal (edge-case: token expired mid-session) */}
      <AuthModal
        isOpen={authOpen}
