...
// Case-insensitive match for the full line (with optional trailing colon)
const HEADER_RE =
...
function parseDescription(html: string | null): React.ReactNode {
  ...
  // flatMap so a block whose first line is a header emits [<header>, <content>]
...
export default function JobDetailPanel({ item, favorited, applied, onClose, onFavorite, onApply }: Props) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div
        className={`fixed right-0 top-0 h-full w-full sm:max-w-[500px] bg-[#0d1117] border-l border-slate-800
