...
function UploadPrompt({ uploading, error, onPickFile }: UploadPromptProps) {
  return (
    <div className="flex flex-col items-center justify-center py-24 gap-6 text-center">
      ...
      <div className="max-w-sm">
        ...
        <p className="text-slate-500 text-sm leading-relaxed">
          JobLens will parse your resume with AI and score every job against your skills and experience.
        </p>
      </div>
      ...
      <button
        onClick={onPickFile}
        disabled={uploading}
        className="inline-flex items-center gap-2.5 px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold text-sm transition-colors"
      >
        {uploading ? (
          <>
...
// ── Resume banner (resume exists, shown above match content) ──────────────────
...
function ResumeBanner({ resume, uploading, onPickFile }: ResumeBannerProps) {
  const sizeKb = Math.round(resume.fileSizeBytes / 1024)
  return (
    <div className="flex items-center gap-3 px-4 py-3 bg-slate-900/60 border border-slate-800/60 rounded-xl">
      ...
      {resume.parsed ? (
        ...
        <span className="text-xs text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-full shrink-0">
          not parsed
        </span>
      )}
      <button
...
// ── Find matches prompt (resume exists but no run yet) ────────────────────────
...
function FindMatchesPrompt({ onRun }: { onRun: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 gap-6 text-center">
      ...
      <p className="text-slate-700 text-xs">Takes 1–2 minutes · scored by Claude AI</p>
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col items-center justify-center py-28 gap-5 text-center">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm">This usually takes 1–2 minutes. Hang tight.</p>
      </div>
    </div>
  )
}
...
interface ResultsHeaderProps {
  ...
  minScore: MinScore
  onMinScoreChange: (s: MinScore) => void
  onRerun: () => void
}

function ResultsHeader({ total, displayed, summary, minScore, onMinScoreChange, onRerun }: ResultsHeaderProps) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
      <div className="flex items-baseline gap-3">
        ...
        <span className="text-sm text-slate-500">
          {displayed} shown
          {summary && (
            <> · {summary.scored.toLocaleString()} jobs scored</>
          )}
        </span>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            id="minScore"
            value={minScore}
            onChange={e => onMinScoreChange(Number(e.target.value) as MinScore)}
            className="text-xs text-slate-300 bg-slate-900 border border-slate-800 rounded-lg px-2 py-1.5 focus:outline-none focus:border-indigo-500/50 transition-colors"
          >
        ...
        <button
          onClick={onRerun}
          className="text-xs font-medium text-slate-500 hover:text-slate-300 border border-slate-800 hover:border-slate-700 bg-slate-900 px-3 py-1.5 rounded-lg transition-colors"
        >
          Re-score
        </button>
      </div>
...
export default function MyMatchesPanel({ token }: { token: string }) {
  ...
  const displayedMatches = allMatches.filter(m => m.score >= minScore)

  // On mount: check resume, then load any existing matches
  useEffect(() => {
    let cancelled = false
    async function init() {
      try {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      // Invalidate stale scores from the previous resume so old results never surface
      try { await clearMyMatches(token) } catch { /* ignore — scores will be replaced on next run */ }
      setResume(r)
      setResumeStatus('exists')
      setAllMatches([])
      setRunSummary(null)
  ...
  if (resumeStatus === 'checking') {
    return <Spinner label="Checking your profile…" />
  }
  ...
  return (
    <div>
      ...
      <div className="mt-6">
        ...
        {matchStatus === 'error' && (
          <ErrorCard message={matchError ?? 'Matching failed'} onRetry={handleRun} />
        )}
        {matchStatus === 'done' && (
          <>
            <ResultsHeader
